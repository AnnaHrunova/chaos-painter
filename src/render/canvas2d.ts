...
function drawWorldTrajectory(
  ...
  for (let index = 1; index < visibleSamples.length; index += 1) {
    ...
    ctx.beginPath();
    ctx.moveTo(previous.p3.x * projection.scale, previous.p3.y * projection.scale);
    ctx.lineTo(current.p3.x * projection.scale, current.p3.y * projection.scale);
...
function drawDensityMap(
  ...
  for (let index = startIndex; index <= endIndex; index += stride) {
    ...
    const x = projection.centerX + sample.p3.x * projection.scale;
    const y = projection.centerY + sample.p3.y * projection.scale;
...
function drawPhasePortrait(
  ...
  const xValues = samples.map((sample) => sample.state.theta3);
  const yValues = samples.map((sample) => sample.state.omega3);
  const xBounds = paddedBounds(xValues);
  const yBounds = paddedBounds(yValues);

  drawPlotFrame(ctx, width, height, padding);
  drawAxisLabels(ctx, width, height, padding, 'theta3', 'omega3');

  for (let index = 1; index < samples.length; index += 1) {
    const previous = samples[index - 1];
    const current = samples[index];
    const x1 = padding + normalize(previous.state.theta3, xBounds.min, xBounds.max) * plotWidth;
    const y1 = height - padding - normalize(previous.state.omega3, yBounds.min, yBounds.max) * plotHeight;
    const x2 = padding + normalize(current.state.theta3, xBounds.min, xBounds.max) * plotWidth;
    const y2 = height - padding - normalize(current.state.omega3, yBounds.min, yBounds.max) * plotHeight;
...
function buildMethodDeltaSeries(
  ...
  for (let index = 0; index < length; index += stride) {
    ...
    const dx = current.p3.x - reference.p3.x;
    const dy = current.p3.y - reference.p3.y;
    points.push({
      time: current.time,
...
function getTrajectoryMaxRadius(trajectory: TrajectorySeries): number {
  ...
  const maxRadius =
    trajectory.samples.reduce((largest, sample) => {
      ...
      return Math.max(largest, p1Radius, p2Radius, p3Radius);
    }, 0) || 2;
